...
def crawling_naver(url):
    ...
    time.sleep(0.5)
    WebDriverWait(browser, 10).until(EC.visibility_of_element_located((By.XPATH, '//*[@id="__next"]/div/div[1]/div[10]/div[2]/section/section/div/button[2]'))).click()
    time.sleep(0.5)
...
def main():
    ...
    if not os.path.exists("/app/data"):
        df.to_csv("/app/data/naver_danang_26.csv", index=False, mode = "w", encoding="utf-8")
    else :
        df.to_csv("/app/data/naver_danang_26.csv", index=False, mode = "a", encoding="utf-8", header=False)
