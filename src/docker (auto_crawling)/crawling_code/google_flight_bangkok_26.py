...
def get_osaka_data():
    ...
    arrival_text_input[0].send_keys("BKK")
    driver.implicitly_wait(5)
    ...
    for value in tmp_flight_number:
        ...
        tmp_reg = re.search("\w+\w+-\d+(\d+\d+)?", tmp_result[index+18:]).group()
...
def main():
    ...
    if not os.path.exists("/app/data"):
        df.to_csv("/app/data/google_bangkok_26.csv", index=False, mode = "w", encoding="utf-8")
    else :
        df.to_csv("/app/data/google_bangkok_26.csv", index=False, mode = "a", encoding="utf-8", header=False)
